(function () {
  var findings = [
    ...
    { cat: "reuso", date: "2026-09-24", nota: "Nota 21", hus: "HU5 verificada contra HU3",
      title: "HU5: casi toda cubierta por HU3; lo nuevo es que el administrador pueda consultar el historial",
      take: "Los criterios 1-10 ya los cumplía el log que escribe cada carga. El criterio 11 pedía algo que no existía: que el administrador pueda consultar esos logs. Es la primera consulta de auditoría en producción, y cierra el pendiente de la Nota 9.",
      body: "<p><b>Nuevo:</b> <code class=\"mono\">ConsultarHistorialAsync</code> exige sesión de administrador antes de leer y devuelve las cargas de la más reciente a la más antigua, con el nombre del administrador. Pantalla \"Historial de cargas\" con inicio y fin en el formato normativo (JC50/JC51), duración, conteos, resultado y detalle del error. <code class=\"mono\">ContextoCargaBiografica</code> es el único lugar que escribe y lee el contexto del evento.</p><p><b>Justificado:</b> el resultado \"Parcial\" (criterio 8) no puede ocurrir: la carga es todo o nada. Test nuevo de inicio y fin como dos momentos distintos (el de HU3 usaba un reloj detenido). El fake de auditoría compartido ignoraba los filtros de consulta; ahora filtra igual que el repositorio real.</p><div class=\"callout\"><b>Descartado con evidencia:</b> la suite se volvió más lenta. En vez de suponer la causa, se corrió el mismo test de volumen con el código de HU4 y el de HU5 bajo las mismas condiciones: el de HU4 tardó igual o más. Era la máquina cargada (una VM corriendo en paralelo), no una regresión.</div>" },
